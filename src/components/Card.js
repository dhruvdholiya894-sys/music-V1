...
function Card({ element }) {
  ...
  return (
    <div key={element.id} className="col-lg-3 col-md-6 py-2">
      <div className="card">
        <Link to={`/track/${element.id}`} state={{ track: element }} className="ratio ratio-1x1 bg-secondary bg-opacity-25 d-block">
          <img
        ...
        <div className="card-body">
          <h5 className="card-title d-flex justify-content-between">
            <Link to={`/track/${element.id}`} state={{ track: element }} className="text-decoration-none text-dark">
              {element.name}
            </Link>
